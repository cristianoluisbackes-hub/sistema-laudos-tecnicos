  ...
  try {
    ...
    if (analises) {
      for (const analise of analises) {
        if (analise.foto_url) {
          await supabase.storage.from('fotos-laudos').remove([analise.foto_url]);
        }
      }
    }
...
export async function uploadFoto(laudoId, analiseId, file) {
  try {
    ...
    // Caminho: /laudos/{userId}/{laudoId}/analise-{analiseId}.jpg
    const caminhoArquivo = `laudos/${user.id}/${laudoId}/analise-${analiseId}-${Date.now()}.jpg`;
