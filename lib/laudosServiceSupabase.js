...
export async function finalizarLaudo(laudoId, pdfUrl, status = 'approved', idiomaPdf = 'pt-BR') {
  try {
    ...
    console.log('✅ Laudo finalizado:', laudoId, `(idioma: ${idiomaPdf})`);
  } catch (error) {
...
export async function obterEstatisticas() {
  try {
    ...
    const [{ count: total }, { count: aprovados }, { count: reprovados }] = await Promise.all([
      supabase.from('laudos').select('*', { count: 'exact', head: true }).eq('criador_id', user.id),
      supabase.from('laudos').select('*', { count: 'exact', head: true }).eq('criador_id', user.id).eq('status', 'approved'),
      supabase.from('laudos').select('*', { count: 'exact', head: true }).eq('criador_id', user.id).eq('status', 'rejected'),
    ]);

    const t = total ?? 0;
    const a = aprovados ?? 0;
    const r = reprovados ?? 0;
    const taxa = t > 0 ? Math.round((a / t) * 100) : 0;

    return { total: t, aprovados: a, reprovados: r, taxa };
  } catch (error) {
...
export async function criarNorma(codigo, descricao = '', specification = '') {
  try {
    const { data, error } = await supabase
      .from('normas')
      .insert({
    ...
    console.log('✅ Norma criada:', codigo);
    return data[0];
  } catch (error) {
...
export async function atualizarNorma(id, codigo, descricao = '', specification = '') {
  try {
    const { error } = await supabase
      .from('normas')
      .update({
    ...
    console.log('✅ Norma atualizada:', id);
  } catch (error) {
...
export async function criarTemplate(nome, descricao = '', cor = 'blue') {
  try {
    ...
    console.log('✅ Template criado:', nome);
    return data[0];
  } catch (error) {
...
export async function clonarTemplate(id, novoNome) {
  try {
    ...
    console.log('✅ Template clonado:', novoTemplate.id);
    return novoTemplate;
  } catch (error) {
...
export async function adicionarAnaliseTemplate(
  ...
  try {
    ...
    console.log('✅ Análise adicionada ao template');
    return data[0];
  } catch (error) {
    console.error('❌ Erro ao adicionar análise:', error.message);
    throw error;
...
export async function atualizarAnaliseTemplate(
  ...
  try {
    ...
    console.log('✅ Análise do template atualizada');
  } catch (error) {
    console.error('❌ Erro ao atualizar análise:', error.message);
    throw error;
  }
...
export async function deletarAnaliseTemplate(id) {
  try {
    ...
    console.log('✅ Análise deletada do template');
  } catch (error) {
    console.error('❌ Erro ao deletar análise:', error.message);
    throw error;
  }
...
export async function listarCatalogoAnalises(filtro = '') {
  try {
    let query = supabase
      ...
      .select(`
        id, nome, specification, tipo_foto,
        norma:norma_id ( id, codigo, descricao, specification )
      `)
      .eq('ativo', true)
...
export async function criarCatalogoAnalise(nome, normaId = null, specification = '', tipoFoto = 'optional') {
  try {
    const { data, error } = await supabase
      .from('catalogo_analises')
      .insert({
...
export async function atualizarCatalogoAnalise(id, nome, normaId = null, specification = '', tipoFoto = 'optional') {
  try {
    const { error } = await supabase
      .from('catalogo_analises')
      .update({
...
export async function reordenarAnaliseTemplate(analiseId, novaOrdem) {
  try {
    ...
    console.log('✅ Análise reordenada');
  } catch (error) {
